...
Book.find({rating: {$lt: 5}}).sort({author: 1}).exec(function (error, result) {
    console.log(result)
});


//3. Find all the Fiction books, skip the first 2, and display only 3 of them
Book.find({genres: ["fiction"]}).skip(2).limit(3).exec(function (error, result) {
    console.log(result)
...
Person.find({$or: [{hair: "gray"}, {eyes: "gray"}]}, ({weight: {$lt: 70}}), function (error, result) {
    console.log(result)
});

//4. Find people who have at least 1 kid with grey hair
//solo los padres de cabello gris con hijos de cabello gris
Person.find().and([{hair:"grey"}, ({kids: {$elemMatch: {hair: "grey"}}})]).exec(function (error, result) {
    console.log(result)
});
...
Person.find().and([
    {weight: {"$gt": 100}}, ({kids: {$elemMatch: {weight: {$gt: 100}}}})]).exec(function (error, result) {
        console.log(result)
    });